...
import ISTP.domain.help.faq.FaqTypeName;
...
public class InitData {
    ...
    static class InitService {
        ...
        public void init() {
            ...
            FaqTypeCategories faqTypeCategories1 = new FaqTypeCategories(FaqTypeName.PARTICIPATION);
            FaqTypeCategories faqTypeCategories2 = new FaqTypeCategories(FaqTypeName.COMMON_SENSE);
            FaqTypeCategories faqTypeCategories3 = new FaqTypeCategories(FaqTypeName.ACCOUNT);
            FaqTypeCategories faqTypeCategories4 = new FaqTypeCategories(FaqTypeName.PROGRAM);
            em.persist(faqTypeCategories1);
            em.persist(faqTypeCategories2);
            em.persist(faqTypeCategories3);
            em.persist(faqTypeCategories4);

            Faq faq1 = new Faq("Q 1. 헌혈을 하려면 신분증이 꼭 필요한가요?", "A 1. 혈액관리법 시행규칙에 따라 개인신상정보가 확인된 분들만 헌혈에 참여하실 수 있도록 헌혈 전에 신분증을 확인합니다. 신분증을 확인함으로써 헌혈자는 헌혈기록 및 검사결과의 정확한 관리가 가능하고 수혈자는 대리헌혈 및 검사목적의 헌혈이 방지되어 안전한 혈액을 수혈 받을 수 있습니다.", FaqTypeName.PARTICIPATION_ID);
            Faq faq2 = new Faq("Q 2. 지정 헌혈이란 무엇인가요?", "A 2. 의료기관 및 환자가 지정 의뢰한 헌혈지원자가 대한적십자사 혈액원에서 헌혈 후 그 혈액을 지정된 수혈자에게 수혈되는 헌혈을 말합니다. 본인이 지정헌혈자인 경우 의료기관이 발급한 '지정헌혈 안내문'을 환자 또는 보호자로부터 문자로 전송 받거나 혹은 출력물을 받아 지참하고 헌혈의집을 방문하여 주시기 바랍니다.\n" +
                    "\n" +
                    "* 지정헌혈 안내문에는 수혈자 등록번호 등 환자에게 필요한 정보를 포함하고 있으며, 지정헌혈 안내문을 지참하지  않은 경우에는 지정헌혈이 불가하니, 반드시 환자 또는 보호자에게 '지정헌혈 안내문'을 받아 오시는 것이 중요합니다.", FaqTypeName.PARTICIPATION_ID);
            Faq faq3 = new Faq("Q 3. 헌혈하기 전에 주의해야 할 점은 무엇인가요?", "A 3. 과음과 과로를 하지 않고 제때에 식사를 한 상태가 좋습니다. 헌혈 전에 적어도 72시간 동안은 혈소판 기능을 저하시키는 아스피린, 치료목적의 약물을 복용하지 않아야 하며, 전신상태가 양호한 경우에 헌혈에 참여하실 수 있습니다. 단, 복용하신 약물의 종류나 기타 건강상태에 따라 헌혈 유보 기간이 달라질 수 있습니다.", FaqTypeName.PARTICIPATION_ID);
            ...
            Faq faq6 = new Faq("Q. 3. 성분 헌혈이 무엇인가요?", "A. 3. 혈액의 성분 중 필요한 성분만 채취하는 헌혈로, 혈액을 일정량 뽑은 후, 원심 분리기를 이용하여, 필요한 성분을 여과한 후, 나머지 성분은 생리식염수에 섞어, 헌혈자에게 다시 돌려주는 헌혈입니다. 헌혈 시간은 대체로 30분~90분 이내이며, 전혈에 비해 신체적인 부담이 적은 편입니다. 일정 성분만 얻어내기 때문에, 회복 속도가 빨라 성분 헌혈의 주\n" +
                    "기는 2주이며, 연 24회까지 진행할 수 있습니다. 성분 헌혈은 혈장, 혈소판, 혈장+혈소판 헌혈로 구분되는데, 혈소판 성분 헌혈은 혈액의 혈소판을 채집하고, 혈소판 혈장 성분 헌혈은 혈액의 혈장과 혈소판을 동시에 채집합니다. 또한, 혈장 성분 헌혈은 혈액의 성분 내 혈장을 채집합니다.", FaqTypeName.COMMON_SENSE_ID);

            Faq faq7 = new Faq("Q 1. 회원탈퇴는 어떻게 할 수 있나요?", "A 1. 로그인을 하신 후, 마이페이지 - 회원탈퇴 메뉴를 클릭하시면 됩니다. ", FaqTypeName.ACCOUNT_ID);
            Faq faq8 = new Faq("Q 2. 이제까지 시행한 지정 헌혈의 횟수는 어떻게 확인할 수 있나요?", "A 2. 로그인 하신 후, 마이페이지 - 헌혈 정보 탭에 들어가시면, 누적 헌혈 횟수를 확인하실 수 있습니다.", FaqTypeName.ACCOUNT_ID);
            Faq faq9 = new Faq("Q 3. 헌혈 횟수가 달라요~", "A 3. 헌혈횟수에 대한 오류 원인으로는 전자문진 시행 이전에 헌혈자들이 기록한 ‘헌혈기록카드’의 판독 오류와 주민등록번호 뒷자리 미기록에서 발생하는 경우가 가장 많습니다. 현재, 누락된 헌혈횟수가 있으시면 가까운 혈액원으로 연락해 주시기 바랍니다.", FaqTypeName.ACCOUNT_ID);

            Faq faq10 = new Faq("Q 1. 로그인이 안돼요", "A 1. 홈페이지를 새로 고침하거나, 새 인터넷 창을 통해, 홈페이지에서 로그인을 시도해보시고, 이로도 해결되지 않으면, 1:1 고객센터로 문의해주세요.", FaqTypeName.PROGRAM_ID);
            Faq faq11 = new Faq("Q 2. 헌혈 횟수가 많은데, 명예의 전당에 이름이 안 올라와요!", "A 2. 명예의 전당은 사용자의 마이 페이지에 기록된 누적 횟수의 크기에 따라, Top 100을 선정하여, 명예의 전당에 이름을 올려드리고 있습니다. 명예의 전당에 이름이 없다면, 고객님의 헌혈 횟수가 부족한 것으로 추정됩니다.", FaqTypeName.PROGRAM_ID);
